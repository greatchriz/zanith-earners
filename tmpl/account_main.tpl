...
{block name=content}
   <main class="main-content w-full px-[var(--margin-x)] pb-8">
      ...
      {if $userinfo.tfa_not_enabled}
         <b>Security Note: please, activate <a href="{"?a=security"|encurl}">Two Factor Authentication</a> to keep your account
            safe.</b>
      {/if}
      ...
      >
         ...
         {if $last_deposit}
            <tr>
               <td>Last Deposit:</td>
               <td>{$currency_sign}<b>{$last_deposit|default:"n/a"}</b> &nbsp; <small>{$last_deposit_date|default:"n/a"}</small>
               </td>
            </tr>
         {/if}
         ...
         {if $last_withdrawal}
            <tr>
               ...
               <td>{$currency_sign}<b>{$last_withdrawal|default:"n/a"}</b> &nbsp;
                  <small>{$last_withdrawal_date|default:"n/a"}</small></td>
            </tr>
         {/if}
